...
def insert_usercard(connection: Connection, card_name: str, user_id: str) -> CardResult:
    cursor = connection.cursor()
...
def remove_usercard(connection: Connection, card_name:str, user_id:str) -> CardResult:
    cursor = connection.cursor()
    if ('"' in user_id+card_name) or ("'" in user_id+card_name) :
        print('SQL 문법 오류가 발생하였습니다.')
        return CardResult.SQL_INJECTED
